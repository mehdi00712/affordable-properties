// Cloudinary config from your environment
...
export async function uploadFilesToCloudinary(files){
  ...
  if (!cloudinaryReady()) {
    console.warn("Cloudinary not configured. Submitting without images.");
    return [];
  }
  const urls=[];
  for (const file of files){
    ...
    if (!res.ok){
      ...
      throw new Error(`Cloudinary upload failed (${res.status}): ${t.slice(0,180)}`);
    }
