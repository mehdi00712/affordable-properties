...
async function render(filters = {}) {
  ...
  if (items.length === 0) { grid.innerHTML = '<p style="padding:8px 16px">No results.</p>'; return; }
  ...
  for (const it of items) {
    ...
    a.querySelector('.card-meta').textContent =
      `${it.propertyType} • ${it.city}, ${it.country} • ${it.bedrooms||0} bed • ${it.bathrooms||0} bath • ${it.sizeSqm||'-'} sqm`;
    a.querySelector('.card-price').textContent =
      currencyFmt(it.price, it.currency) + (it.type==='rent' ? ' / mo' : '');
    grid.appendChild(a);
  }
}
