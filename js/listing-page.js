...
async function load(){
  ...
  if (!snap.exists()){ document.body.innerHTML = '<main class="wrap"><p>Listing not available.</p></main>'; return; }
  const l = snap.data();

  if (l.status!=='approved'){
    let allowed=false; await new Promise(r=>onAuthStateChanged(auth,u=>{ allowed=!!u; r(); }));
    if (!allowed){ document.body.innerHTML = '<main class="wrap"><p>Listing not available.</p></main>'; return; }
  }
  ...
  document.getElementById('btn-report').onclick = async ()=>{
    let uid='anon'; await new Promise(r=>onAuthStateChanged(auth,u=>{ if(u) uid=u.uid; r(); }));
